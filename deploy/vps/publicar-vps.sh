#!/usr/bin/env bash
# Publica AgroNota na VPS com Traefik + Let's Encrypt + systemd.
#
# Uso:
#   ./deploy/vps/publicar-vps.sh
#   VPS_PASS='senha' ./deploy/vps/publicar-vps.sh
#
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
VPS_HOST="${VPS_HOST:-root@187.77.41.55}"
...
# Segredos produção — defaults, depois remoto (redeploy), depois .env local
...
if ssh_cmd "test -f ${REMOTE_DIR}/portal-api/.env" 2>/dev/null; then
  echo "==> Redeploy: preservando segredos existentes na VPS..."
  REMOTE_ENV_FILE=$(ssh_cmd "cat ${REMOTE_DIR}/portal-api/.env")
  extract_env() {
    echo "$REMOTE_ENV_FILE" | grep -E "^${1}=" | head -1 | cut -d= -f2- || true
  }
  DB_PASS=$(extract_env DB_PASSWORD)
  [[ -z "$DB_PASS" ]] && DB_PASS=$(openssl rand -hex 16)
  JWT_SECRET=$(extract_env NFSE_JWT_SECRET)
  [[ -z "$JWT_SECRET" ]] && JWT_SECRET=$(openssl rand -hex 32)
  ADMIN_SECRET=$(extract_env NFSE_ADMIN_SECRET)
  [[ -z "$ADMIN_SECRET" ]] && ADMIN_SECRET=$(openssl rand -hex 16)
...
# Reutiliza Stripe/Mail do .env local se existir (sobrescreve remoto)
if [[ -f "$ROOT/portal-api/.env" ]]; then
  # shellcheck disable=SC1091
  source "$ROOT/portal-api/.env"
fi

echo "==> Preparando VPS (remove n8n/jira/sonar, instala deps)..."
ssh_cmd bash -s <<REMOTE_PREP
set -euo pipefail
export DEBIAN_FRONTEND=noninteractive

echo "--- Removendo stacks antigos ---"
docker stack rm n8n 2>/dev/null || true
docker stack rm jira-sonar 2>/dev/null || true
sleep 8

echo "--- Instalando Java 21, Node 20, MariaDB ---"
apt-get update -qq
apt-get install -y -qq curl ca-certificates gnupg rsync wget apt-transport-https mariadb-server

# Temurin JDK 21 (Debian 12 não tem openjdk-21 nos repos padrão)
...
mkdir -p /opt/agrownota/portal-api /opt/agrownota/portal-web /var/log/agrownota
...
mysql -e "CREATE USER IF NOT EXISTS 'nfse'@'localhost' IDENTIFIED BY '${DB_PASS}';"
...
echo "==> npm ci + build produção (portal-web)..."
...
echo ""
echo "============================================"
echo "  AgroNota publicado"
echo "============================================"
echo "  URL:  https://${DOMAIN}/"
echo "  SSL:  Let's Encrypt via Traefik (renovação automática)"
echo "  Logs: ssh ${VPS_HOST} 'tail -f /var/log/agrownota/*.log'"
echo "  Svc:  systemctl status agrownota-api agrownota-web"
echo "============================================"

sleep 8
curl -sfI "https://${DOMAIN}/login" | head -5 || echo "(aguarde ~60s para certificado SSL propagar)"
